...
def auto_repr(cls):
    ...
    if "__init__" not in members:
        raise TypeError(f"{cls.__name__} already defines __init__")
    ...
    if not all(
        ...
        raise TypeError(
            f"Cannot apply auto_repr to {cls.__name} because not all "
            "__init__ parametes have matching properties"
        )
